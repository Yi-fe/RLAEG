...
class End2EndModel(Module):
	...
	def path_to_exe_vector(cls, path: str, max_length: int, padding_value: int, shift_values: bool) -> np.ndarray:
		...
		"""
		exe = cls.load_sample_from_file(path, max_length, has_shape=False, padding_value=padding_value,
	...
	def bytes_to_numpy(cls, bytez: bytes, max_length: int, padding_value: int, shift_values: bool) -> np.ndarray:
		...
		b[: len(bytez)] = bytez
		return np.array(b, dtype=float).reshape(max_length)
	...
	def forward(self, bytez: bytes) -> torch.Tensor:
		"""
		Forward pass.
		...
		torch.Tensor
			the result of the forward pass
		"""
		x = self.bytes_to_numpy(bytez, 2 ** 20, 256, False)
		x = torch.tensor(x)
	...
	def load_simplified_model(self, path: str):
		...
		"""
		if not isfile(path):
			raise Exception('{} path not pointing to regular file!'.format(path))
