...
def getParams():
    ...
    params = {
        'channels'     : ("suggest_int", {'name':'channels', 'low':48, 'high':48}),
        'window_size'  : ("suggest_int", {'name':'window_size', 'low':32, 'high':32}),
    }
...
class AvastConv(LowMemConvBase):
    ...
    def __init__(self, out_size=2, channels=48, window_size=32, embd_size=8, stride=4):
        super(AvastConv, self).__init__()
        self.embd = nn.Embedding(257, embd_size, padding_idx=0)
        for i in range(1, 257):
            self.embd.weight.data[i, :] = torch.tensor(vec_bin_array(np.asarray([i]), m=embd_size))
